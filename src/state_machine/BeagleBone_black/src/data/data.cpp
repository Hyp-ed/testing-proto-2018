...
const char *states[num_states] = {
    "Idle",         "Calibrating",      "Ready",       "Accelerating",
    "Decelerating", "EmergencyBraking", "RunComplete", "FailureStopped",
    "Exiting",      "Finished",
};
...
void Data::setSensorsImuData(
    const DataPoint<array<Imu, Sensors::kNumImus>> &imu) {
  ScopedLock L(&lock_sensors_);
  sensors_.imu = imu;
}
...
void Data::setEmergencyBrakesData(
    const EmergencyBrakes &emergency_brakes_data) {
  ScopedLock L(&lock_emergency_brakes_);
