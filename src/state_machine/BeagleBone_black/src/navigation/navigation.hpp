...
constexpr std::array<NavigationType, 42> kStripeLocations = {
    0.0,    30.48,   60.96,   91.44,   121.92,  152.4,   182.88,
    213.36, 243.84,  274.32,  304.8,   335.28,  365.76,  396.24,
    426.72, 457.2,   487.68,  518.16,  548.64,  579.12,  609.6,
    640.08, 670.56,  701.04,  731.52,  762.0,   792.48,  822.96,
    853.44, 883.92,  914.4,   944.88,  975.36,  1005.84, 1036.32,
    1066.8, 1097.28, 1127.76, 1158.24, 1188.72, 1219.2,  1249.68};
...
class Navigation {
 ...
 public:
  ...
  struct Settings {
    ...
    float prox_orient_w = 0.1;  ///< Weight (from [0,1]) of proxi vs imu in
                                ///< orientation calculation
    float prox_displ_w = 0.1;   ///< Weight (from [0,1]) of proxi vs imu in
                                ///< displacement calculation
    float strp_displ_w = 1.0;   ///< Weight [0,1] of stripe count vs imu in
                                ///< displacement calculation
    float prox_vel_w =
        0.01;  ///< Weight (from [0,1]) of proxi vs imu in velocity calculation
    float strp_vel_w =
        0.0;  ///< Weight [0,1]  of stripe count vs imu in velocity calculation
  };
  struct Input {
  ...
  struct FullOutput {
    ...
    const std::array<NavigationVector, Sensors::kNumImus>*
        g;  // Acc offsets (gravitational acc)
    const int* num_gyro_samples;
    const std::array<NavigationVector, Sensors::kNumImus>* gyro_offsets;

    // Most up-to-date values of pod's acceleration, velocity and displacement
    // in 3D
   ...
   *
   * @param post_calibration_barrier Navigation module will wait on this barrier
   * at the end of the transition to 'operational' state. It is primarily meant
   * for syncing with motors module.
   */
  Navigation(Barrier& post_calibration_barrier,
             Logger& log = System::getLogger(),
             std::string file_path =
                 "../BeagleBone_black/data/configuration/NavSettings.txt");

  /**
   * @brief Get the acceleration value
   *
   * @return NavigationType Returns the forward component of acceleration vector
  ...
  /**
   * @brief Transition the navigation module to an operational state so that it
   * starts producing useful output. Hits the `post_calibration_barrier` before
   * returning `true` (to indicate to motors that the calibration is done).
 ...
 private:
  /**
   * @brief Front-right, rear-right, rear-left, and front-left proxi sensor
   * distances. This is likely temporary and will be replaced by an array or
   * other suitable data structure once the algorithm using it is complete.
   */
#ifdef PROXI
  struct Proximities {
    float fr;  // mm
  ...
  /**
   * @brief Calculates distance to the given stripe, the next stripe and the one
   * after that.
   *
   * @param  stripe_count  Number of stripes.
   * @return std::array<NavigationType, 3> Index 0 contains distance to last
   * stripe (should be negative); indices 1 and 2 contain distances to the next
   * 2 stripes (both should be positive).
   */
  std::array<NavigationType, 3> getNearestStripeDists(uint16_t stripe_count);

  /**
   * @brief Updates navigation based on new IMU and stripe counter readings.
   * Should be called when IMU, proximity sensors, and stripe counter have all
   * been updated.
   *
  ...
  void gyroUpdate(
      DataPoint<NavigationVector> angular_velocity);  // Point number 1
  void accelerometerUpdate(
      DataPoint<NavigationVector> acceleration);  // Points 3, 4, 5, 6
#ifdef PROXI
  void proximityOrientationUpdate(Proximities ground,
                                  Proximities rail);  // Point number 7
  void proximityDisplacementUpdate(Proximities ground,
                                   Proximities rail);  // Point number 7
#endif
  void stripeCounterUpdate(StripeCounterArray scs);  // Point number 7
  void opticalEncoderUpdate(
  ...
  std::array<NavigationVector, Sensors::kNumImus>
      g_;  // Acc offsets (gravitational acc)
  int num_gyro_samples_;
  std::array<NavigationVector, Sensors::kNumImus>
      gyro_offsets_;  // Measured during calibration

  // Most up-to-date values of pod's acceleration, velocity and displacement in
  // 3D; used for output
  ...
  DataPoint<NavigationVector>
      prev_angular_velocity_;  // To calculate how much has the pod rotated
  Quaternion<NavigationType>
      orientation_;  // Pod's orientation is updated with every gyro reading

  // Filters for reducing noise in sensor data before processing the data in any
  // other way
  std::array<Kalman<NavigationVector>, Sensors::kNumImus>
      acceleration_filter_;  // One for each IMU
  std::array<Kalman<NavigationVector>, Sensors::kNumImus>
      gyro_filter_;  // One for each IMU
#ifdef PROXI
  std::array<Kalman<float>, 2 * Sensors::kNumProximities> proximity_filter_;
#endif
  Integrator<NavigationVector>
      acceleration_integrator_;  // Acceleration to velocity
  Integrator<NavigationVector>
      velocity_integrator_;  // Velocity to displacement
  Differentiator<NavigationType>
      stripe_differentiator_;  // Stripe cnt distance to velocity
#ifdef PROXI
