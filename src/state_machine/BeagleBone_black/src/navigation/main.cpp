...
Main::Main(uint8_t id, Logger &log)
    : Thread(id, log), data_(data::Data::getInstance()),
...
void Main::run() {
...
#ifdef PROXI
  std::unique_ptr<Navigation::ProximityArray> last_proxis(
      new Navigation::ProximityArray());
  std::unique_ptr<Navigation::ProximityArray> proxis(
      new Navigation::ProximityArray());
  ...
  for (int i = 0; i < Sensors::kNumProximities; ++i) {
    ...
    (*last_proxis)[i + Sensors::kNumProximities] =
        &(last_readings->proxi_back.value[i]);
  }
#endif
  ...
  while (sys.running_) {
    ...
    case State::kIdle:
      *readings = data_.getSensorsData();
      if ((readings->module_status == ModuleStatus::kInit ||
          ...
          nav_.getStatus() ==
              ModuleStatus::kStart) { // NOLINT [whitespace/braces]
    ...
    *readings = data_.getSensorsData();
    ...
    if (readings->imu.timestamp < last_readings->imu.timestamp) {
      log_.ERR("NAV", "new reading has past timestamp %u",
               readings->imu.timestamp);
      yield();
      continue;
    }

    // TODO(Brano): Accelerations and gyros should be in separate arrays in
    // data::Sensors.
...
inline bool Main::stripeCntChanged(const Sensors &old_data,
                                   const Sensors &new_data) {
  for (int i = 0; i < Sensors::kNumKeyence; i++) {
    if (new_data.keyence_stripe_counter[i].count.value !=
        old_data.keyence_stripe_counter[i].count.value) // NOLINT
      return true;
  }
...
inline bool Main::opticalEncDistChanged(const Sensors &old_data,
                                        const Sensors &new_data) {
...
void Main::updateData() {
  ...
  log_.DBG1("NAV",
            "Update: ms=%d, a=(%.2f, %.2f, %.2f), v=(%.2f, %.2f, %.2f), "
            "d=(%.2f, %.2f, %.2f), bd=%.2f, ebd=%.2f, calib=%d", // NOLINT
            nav_.status_, nav_.acceleration_[0], nav_.acceleration_[1],
            nav_.acceleration_[2], nav_.velocity_.value[0],
            nav_.velocity_.value[1], nav_.velocity_.value[2],
            nav_.displacement_.value[0], nav_.displacement_.value[1],
            nav_.displacement_.value[2], nav_.getBrakingDistance(),
            nav_.getEmergencyBrakingDistance(), nav_.num_gravity_samples_);
}
