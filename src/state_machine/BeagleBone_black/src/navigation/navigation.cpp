...
float proxiMean(const Proximity* const a, const Proximity* const b) {
  ...
  if (a->operational) return a->val;
  if (b->operational) return b->val;
  return -1;
}
#endif
...
bool Navigation::startCalibration() {
  if (is_calibrating_) return true;
  if (status_ != ModuleStatus::kInit) return false;
...
bool Navigation::finishCalibration() {
  if (!is_calibrating_ || status_ != ModuleStatus::kReady) return false;
...
void Navigation::proximityUpdate(ProximityArray proxis) {
  ...
  if (settings_.proxi_displ_enable) proximityDisplacementUpdate(ground, rail);
  if (settings_.proxi_orient_enable) proximityOrientationUpdate(ground, rail);
}
#endif
