...
Navigation::Navigation(Barrier &post_calibration_barrier, Logger &log,
    ...
    : post_calibration_barrier_(post_calibration_barrier), log_(log),
      status_(ModuleStatus::kStart), is_calibrating_(false),
      num_gravity_samples_(0), num_gyro_samples_(0),
      acceleration_(0), // TODO(Brano): Should this be g or 0?
      velocity_(0, NavigationVector(0)), displacement_(0, NavigationVector(0)),
      stripe_count_(0), prev_angular_velocity_(0, NavigationVector()),
      orientation_(1, 0, 0, 0), acceleration_integrator_(&velocity_),
...
NavigationType Navigation::getAcceleration() const { return acceleration_[0]; }

NavigationType Navigation::getVelocity() const { return velocity_.value[0]; }
...
NavigationType Navigation::getBrakingDistance() const {
  // A polynomial fit for the braking distance at a specific (normalised)
  // velocity, where kCoeffSlow for < 50m/s and kCoeffFast for > 50m/s because
  // kCoeffFast is inaccurate at < 10m/s but they both agree between ~10 and
  // ~50m/s.
  static constexpr std::array<NavigationType, 16> kCoeffSlow = {
      136.3132, 158.9403,  63.6093,  -35.4894, -149.2755, 152.6967,
      502.5464, -218.4689, -779.534, 95.7285,  621.1013,  50.4598,
      -245.099, -54.5,     38.0642,  12.3548};
  static constexpr std::array<NavigationType, 16> kCoeffFast = {
      258.6,   299.2, 115.2,  -104.7, -260.9, 488.5, 940.8, -808.5,
      -1551.9, 551.7, 1315.7, -61.4,  -551.4, -84.5, 90.7,  26.2};
...
ModuleStatus Navigation::getStatus() const { return status_; }
...
void Navigation::init(SensorCalibration sc, Sensors readings) {
  for (int i = 0; i < Sensors::kNumImus; i++) {
    // TODO(Brano,Uday): Properly initialise filters (with std dev of sensors
    // and stuff)
    ...
    gyro_filter_[i].configure(readings.imu.value[i].gyr,
                              ...
                              NavigationVector(0.1));
    log_.INFO("NAV",
              "IMU[%d]: accl variance = (%.3f, %.3f, %.3f), gyro variance = "
              "(%.3f, %.3f, %.3f)",
              i, sc.imu_variance[i][0][0], sc.imu_variance[i][0][1],
              sc.imu_variance[i][0][1], sc.imu_variance[i][1][0],
              sc.imu_variance[i][1][1], sc.imu_variance[i][1][1]);
  }
#ifdef PROXI
  for (int i = 0; i < Sensors::kNumProximities; ++i) {
    proximity_filter_[i].configure(readings.proxi_front.value[i].val,
                                   sqrt(sc.proxi_front_variance[i]), 0.1);
    proximity_filter_[i + Sensors::kNumProximities].configure(
        readings.proxi_back.value[i].val, sqrt(sc.proxi_back_variance[i]), 0.1);
  ...
  log_.DBG("NAV",
           "After init: a=(%.3f, %.3f, %.3f), v=(%.3f, %.3f, %.3f), d=(%.3f, "
           "%.3f, %.3f)",
           acceleration_[0], acceleration_[1], acceleration_[2],
           velocity_.value[0], velocity_.value[1], velocity_.value[2],
           displacement_.value[0], displacement_.value[1],
           displacement_.value[2]);

  status_ = ModuleStatus::kInit;
}

std::array<NavigationType, 3>
Navigation::getNearestStripeDists(uint16_t stripe_count) {
  std::array<NavigationType, 3> arr;
  for (unsigned int i = 0; i < arr.size(); ++i)
    arr[i] = kStripeLocations[std::min(stripe_count + i,
                                       (unsigned int)kStripeLocations.size())] -
             getDisplacement();
  return arr;
}
...
void Navigation::update(Input input) {
  ...
  if (input.proxis != nullptr && !is_calibrating_ &&
      (settings_.proxi_displ_enable ||
       settings_.proxi_orient_enable)) { // NOLINT[whitespace/braces]
...
void Navigation::imuUpdate(DataPoint<ImuArray> imus) {
  for (unsigned int i = 0; i < imus.value.size(); ++i) {
    log_.DBG3("NAV",
              "Before filtering: a[%d]=(%.3f, %.3f, %.3f), omega[%d]=(%.3f, "
              "%.3f, %.3f)",
              i, imus.value[i].acc[0], imus.value[i].acc[1],
              imus.value[i].acc[2], i, imus.value[i].gyr[0],
              imus.value[i].gyr[1], imus.value[i].gyr[2]);
    ...
    log_.DBG3("NAV",
              " After filtering: a[%d]=(%.3f, %.3f, %.3f), omega[%d]=(%.3f, "
              "%.3f, %.3f)",
              i, imus.value[i].acc[0], imus.value[i].acc[1],
              imus.value[i].acc[2], i, imus.value[i].gyr[0],
              imus.value[i].gyr[1], imus.value[i].gyr[2]);
  ...
  if (num_operational < 2) {
    ...
    log_.ERR("NAV", "Critical failure: num operational IMUs = %d < 2",
             num_operational);
  ...
  if (is_calibrating_) {
    ...
    accelerometerUpdate(
        DataPoint<NavigationVector>(imus.timestamp, acc / num_operational));
    if (settings_.gyro_enable)
      gyroUpdate(
          DataPoint<NavigationVector>(imus.timestamp, gyr / num_operational));
  }
}
#ifdef PROXI
void Navigation::proximityUpdate(ProximityArray proxis) {
  Proximities ground, rail;
  // TODO(Brano,Martin): Make sure proxis are in correct order (define index
  // constants)
  int num_ground_fail = 0;
  if ((ground.fr = proxiMean(proxis[6], proxis[7])) < 0)
    ++num_ground_fail;
  if ((ground.rr = proxiMean(proxis[8], proxis[9])) < 0)
    ++num_ground_fail;
  if ((ground.rl = proxiMean(proxis[14], proxis[15])) < 0)
    ++num_ground_fail;
  if ((ground.fl = proxiMean(proxis[0], proxis[1])) < 0)
    ++num_ground_fail;
  ...
  if (num_ground_fail > 1) {
    status_ = ModuleStatus::kCriticalFailure;
    log_.ERR("NAV", "Critical failure: num failed ground proxi points = %d",
             num_ground_fail);
    return;
  }
...
void Navigation::calibrationUpdate(ImuArray imus) {
  ...
  for (unsigned int i = 0; i < data::Sensors::kNumImus; ++i) {
    ...
    gyro_offsets_[i] =
        gyro_offsets_[i] + (imus[i].gyr - gyro_offsets_[i]) / num_gyro_samples_;
  }
...
void Navigation::gyroUpdate(DataPoint<NavigationVector> angular_velocity) {
  ...
  double angle_of_rotation =
      (angular_velocity.timestamp - prev_angular_velocity_.timestamp) * theta /
      2;
  double scalar_part = cos(angle_of_rotation);
  auto vector_part =
      (prev_angular_velocity_.value / theta) * sin(angle_of_rotation);
...
void Navigation::accelerometerUpdate(DataPoint<NavigationVector> acceleration) {
  log_.DBG2("NAV",
            "Before accl update: a=(%.3f, %.3f, %.3f), v=(%.3f, %.3f, %.3f), "
            "d=(%.3f, %.3f, %.3f)",
            acceleration_[0], acceleration_[1], acceleration_[2],
            velocity_.value[0], velocity_.value[1], velocity_.value[2],
            displacement_.value[0], displacement_.value[1],
  ...
  // TODO(Brano): Rotate acceleration if gyro enabled. Need to add conj to
  // Quaternion.
  ...
  if (std::abs(dists[2]) < std::abs(dists[1])) {
    if (status_ != ModuleStatus::kCriticalFailure)
      log_.ERR("NAV",
               "Critical failure: missed stripe (oldCnt=%d, "
               "nearestStripes=[%f, %f, %f])",
               stripe_count_, dists[0], dists[1], dists[2]);
    status_ = ModuleStatus::kCriticalFailure;
  }

  log_.DBG2("NAV",
            " After accl update: a=(%.3f, %.3f, %.3f), v=(%.3f, %.3f, %.3f), "
            "d=(%.3f, %.3f, %.3f)",
            acceleration_[0], acceleration_[1], acceleration_[2],
            velocity_.value[0], velocity_.value[1], velocity_.value[2],
            displacement_.value[0], displacement_.value[1],
...
#ifdef PROXI
void Navigation::proximityOrientationUpdate(Proximities ground,
                                            Proximities rail) {
  // Ground points
  NavigationVector a = kGroundProxiRR;
  a[2] -= ground.rr;
  NavigationVector b = kGroundProxiFR;
  b[2] -= ground.fr;
  NavigationVector c = kGroundProxiFL;
  c[2] -= ground.fl;
  NavigationVector d = kGroundProxiRL;
  d[2] -= ground.rl;

  // Rail points
  NavigationVector e_l = kRailProxiRL;
  e_l[1] -= rail.rl;
  NavigationVector e_r = kRailProxiRR;
  e_r[1] += rail.rr;
  NavigationVector f_l = kRailProxiFL;
  f_l[1] -= rail.fl;
  NavigationVector f_r = kRailProxiFR;
  f_r[1] += rail.fr;
  ...
  m << a[0], b[0], c[0], d[0], a[1], b[1], c[1], d[1], a[2], b[2], c[2], d[2];
  ...
  // Calculate rejection of r on n and use cross product to complete the basis
  // of tube ref. frame
  ...
  Eigen::Quaternion<double> orientation(orientation_[0], orientation_[1],
...
void Navigation::proximityDisplacementUpdate(Proximities ground,
                                             Proximities rail) {
  log_.DBG2("NAV",
            "Before proxi displ update: a=(%.3f, %.3f, %.3f), v=(%.3f, %.3f, "
            "%.3f), d=(%.3f, %.3f, %.3f)",
            acceleration_[0], acceleration_[1], acceleration_[2],
            velocity_.value[0], velocity_.value[1], velocity_.value[2],
            displacement_.value[0], displacement_.value[1],
  ...
  // TODO(Brano): Make this a weighted average based on the actual positions of
  // the 4 sensors with
  //              respect to IMUs
  ...
  // The y-axis points to the left and displacement 0 is when right and left
  // proxis are equal
  // TODO(Brano): Make this a weighted average based on the actual proxi
  // positions w.r.t. IMUs
  ...
  log_.DBG2("NAV",
            " After proxi displ update: a=(%.3f, %.3f, %.3f), v=(%.3f, %.3f, "
            "%.3f), d=(%.3f, %.3f, %.3f)",
            acceleration_[0], acceleration_[1], acceleration_[2],
            velocity_.value[0], velocity_.value[1], velocity_.value[2],
            displacement_.value[0], displacement_.value[1],
...
void Navigation::stripeCounterUpdate(StripeCounterArray scs) {
  log_.DBG2("NAV",
            "Before stripe update: a=(%.3f, %.3f, %.3f), v=(%.3f, %.3f, %.3f), "
            "d=(%.3f, %.3f, %.3f)",
            acceleration_[0], acceleration_[1], acceleration_[2],
            velocity_.value[0], velocity_.value[1], velocity_.value[2],
            displacement_.value[0], displacement_.value[1],
  ...
  if (scs[0].count.value <= stripe_count_ &&
      scs[1].count.value <= stripe_count_) {
  ...
  for (unsigned int i = 0; i < data::Sensors::kNumKeyence; ++i) {
    if (scs[i].operational) {
      // This operational check could be redundant since keyence is always
      // operational (for now)
      ++num_operational;
    }
  }
  ...
  if (scs[0].count.value == scs[1].count.value) {
    ...
    if (std::abs(dists[0]) < std::abs(dists[1]) ||
        std::abs(dists[2]) < std::abs(dists[1])) {
      // Ideally, we'd have dists[1]==0 but if dists[1] is not the closest
      // stripe, something has definitely gone wrong.
      status_ = ModuleStatus::kCriticalFailure;
      log_.ERR("NAV",
               "Critical failure: missed stripe (oldCnt=%d, newCnts=[%d, %d], "
               "nearestStripes=[%f, %f, %f])", // NOLINT
               stripe_count_, scs[0].count.value, scs[1].count.value, dists[0],
               dists[1], dists[2]);
      return;
    }
    ...
    // Distance given by the current stripe should match distance, therefore the
    // dists array should provide [-ve, ~0, +ve] if the stripe count and
    // distance are in agreement
    ...
    if (std::abs(dists_l[0]) < std::abs(dists_l[1]) ||
      ...
      if (std::abs(dists_r[0]) < std::abs(dists_r[1]) ||
        ...
        log_.ERR(
            ...
            stripe_count_, scs[0].count.value, scs[1].count.value, dists_l[0],
            dists_l[1], dists_l[2], dists_r[0], dists_r[1], dists_r[2]);
        return;
      } else {
        ...
        log_.DBG3("NAV",
                  "Keyences are not in sync (oldCnt=%d, newCnts=[%d, %d])",
                  stripe_count_, scs[0].count.value, scs[1].count.value);
      }
    } else {
  ...
  displacement_.value[0] =
      (1 - settings_.strp_displ_w) * displacement_.value[0] +
      settings_.strp_displ_w * dp.value;

  // Update x-axis velocity
  // velocity_.value[0] = (1 - settings_.strp_vel_w) * velocity_.value[0] +
  //                           settings_.strp_vel_w  *
  //                           stripe_differentiator_.update(dp).value;

  log_.DBG2("NAV",
            " After stripe update: a=(%.3f, %.3f, %.3f), v=(%.3f, %.3f, %.3f), "
            "d=(%.3f, %.3f, %.3f)",
            acceleration_[0], acceleration_[1], acceleration_[2],
            velocity_.value[0], velocity_.value[1], velocity_.value[2],
            displacement_.value[0], displacement_.value[1],
...
void Navigation::opticalEncoderUpdate(
    array<float, Sensors::kNumOptEnc> optical_enc_distance) {
