 ...
 * Date: 5/05/18
 * Description:
 * A controller object represents one motor controller. Object is used to
 * request data specific to the controller.
 *
 ...
 * used here are Service Data Object Messages, Network Management Messages and
 * Emergency Messages. All messages are little edian.
 */

/* Service Data Object (SDO) messages provide access to the object dictionary of
 * the controller for reading/writing to the memory register. SDO CAN Frames are
 * composed as follows: CAN ID:    COB-ID = Function + Node ID, Len:       Size
 * of object dictionary entry Byte 0:    Command Bytes 1-2: Object Dictionary
 * Index Byte  3:   Object Dictionary Sub-Index Bytes 4-7: Data
 */

/* The Network Management (NMT) Protocol is used to start/stop and reset the
 * Controller nodes in the system. NMT CAN Frames composed as follows: CAN ID:
 * 0x000 Len:    2 Byte 1: NMT Function Byte 2: Target Node
 */
...
// constexpr uint8_t  kWriteThreeBytes       = 0x27;  // TODO(anyone) add back
// in if needed
constexpr uint8_t kWriteFourBytes = 0x23;

// Network management commands
constexpr uint8_t kNmtOperational = 0x01;
// constexpr uint8_t  kNmtStop               = 0x02;  // TODO(anyone) add back
// in if needed constexpr uint8_t  kNmtPreOperational     = 0x80;  //
// TODO(anyone) add back in if needed constexpr uint8_t  kNmtResetNode = 0x81;
// // TODO(anyone) add back in if needed constexpr uint8_t  kNmtResetComms =
// 0x82;  // TODO(anyone) add back in if needed

Controller::Controller(Logger &log, uint8_t id)
    : log_(log), can_(Can::getInstance()), data_(data::Data::getInstance()),
      motor_data_(data_.getMotorData()), state_(kNotReadyToSwitchOn),
      node_id_(id), critical_failure_(false), actual_velocity_(0),
      actual_torque_(0), sdo_frame_recieved_(false), motor_temperature_(0),
...
bool Controller::hasId(uint32_t id, bool extended) {
  if (kEmgyTransmit + node_id_ == id)
    return true;
  if (kSdoTransmit + node_id_ == id)
    return true;
  if (kNmtTransmit + node_id_ == id)
    return true;
  return false;
}

void Controller::registerController() { can_.registerProcessor(this); }
...
void Controller::configure() {
  ...
  log_.DBG1("MOTOR", "Controller %d: Configuring under voltage limit",
            node_id_);
  sendSdoMessage(sdo_message_);
  if (critical_failure_) {
    return;
  ...
  log_.DBG1("MOTOR", "Controller %d: Configuring under voltage minimum",
            node_id_);
  sendSdoMessage(sdo_message_);
  if (critical_failure_) {
    return;
  ...
  log_.DBG1("MOTOR", "Controller %d: Configuring motor temperature sensor",
            node_id_);
  sendSdoMessage(sdo_message_);
  if (critical_failure_) {
    return;
  ...
  log_.DBG1("MOTOR", "Controller %d: Configuring motor rated current",
            node_id_);
  sendSdoMessage(sdo_message_);
  if (critical_failure_) {
    return;
  ...
  log_.DBG1("MOTOR", "Controller %d: Configuring current control torque P gain",
            node_id_);
  sendSdoMessage(sdo_message_);
  if (critical_failure_) {
    return;
  ...
  log_.DBG1("MOTOR", "Controller %d: Configuring current control torque I gain",
            node_id_);
  sendSdoMessage(sdo_message_);
  if (critical_failure_) {
    return;
  ...
  log_.DBG1("MOTOR", "Controller %d: Configuring current control flux P gain",
            node_id_);
  sendSdoMessage(sdo_message_);
  if (critical_failure_) {
    return;
  ...
  log_.DBG1("MOTOR", "Controller %d: Configuring current control ramp",
            node_id_);
  sendSdoMessage(sdo_message_);
  if (critical_failure_) {
    return;
  ...
  log_.DBG1("MOTOR", "Controller %d: Configuring maximum controller current",
            node_id_);
  sendSdoMessage(sdo_message_);
  if (critical_failure_) {
    return;
  ...
  log_.DBG1("MOTOR", "Controller %d: Configuring secondary current protection",
            node_id_);
  sendSdoMessage(sdo_message_);
  if (critical_failure_) {
    return;
...
void Controller::enterOperational() {
  // Send NMT Operational message to transition from state 0 (Not ready to
  // switch on) to state 1 (Switch on disabled)
  nmt_message_.data[0] = kNmtOperational;
  nmt_message_.data[1] = node_id_;

  log_.INFO("MOTOR", "Controller %d: Sending NMT Operational command",
            node_id_);
  ...
  // Send enter operational message to transition from state 2 (Ready to switch
  // on) to state 4 (Operation enabled)
  sdo_message_.data[0] = kWriteTwoBytes;
  sdo_message_.data[1] = 0x40;
  sdo_message_.data[2] = 0x60;
  sdo_message_.data[3] = 0x00;
...
void Controller::sendTargetVelocity(int32_t target_velocity) {
  ...
  log_.DBG2("MOTOR", "Controller %d: Updating target velocity to %d", node_id_,
            target_velocity);
...
void Controller::sendTargetTorque(int16_t target_torque) {
  ...
  log_.DBG2("MOTOR", "Controller %d: Updating target torque tp %d", node_id_,
            target_torque);
  sendSdoMessage(sdo_message_);
}
...
int32_t Controller::getVelocity() { return actual_velocity_; }

int16_t Controller::getTorque() { return actual_torque_; }
...
bool Controller::getFailure() { return critical_failure_; }

uint8_t Controller::getId() { return node_id_; }

ControllerState Controller::getControllerState() { return state_; }
...
uint8_t Controller::getMotorTemp() { return motor_temperature_; }

uint8_t Controller::getControllerTemp() { return controller_temperature_; }
...
void Controller::sendSdoMessage(utils::io::can::Frame &message) {
  ...
  for (send_counter = 0; send_counter < 3; send_counter++) {
    ...
    if (sdo_frame_recieved_) {
      ...
      log_.DBG1("MOTOR", "Controller %d: No response. Sending SDO frame again",
                node_id_);
    }
  }
...
void Controller::requestStateTransition(utils::io::can::Frame &message,
  ...
  if (state_ != state) {
    throwCriticalFailure();
    log_.ERR("MOTOR", "Controller %d, Could not transition to state %d",
             node_id_, state);
    return;
  }
}
...
void Controller::processEmergencyMessage(utils::io::can::Frame &message) {
  ...
  if (index_2 == 0x23) {
    log_.ERR("MOTOR", "Controller %d: Current on device ouput side error",
             node_id_);
  }
  ...
  if (index_2 == 0x84) {
    log_.ERR("MOTOR", "Controller %d: Velocity speed controller error",
             node_id_);
  }
...
void Controller::processErrorMessage(uint16_t error_message) {
  ...
  case 0x3210:
    ...
    break;
  case 0xFF01:
    log_.ERR("MOTOR", "Controller %d error: Phase A current measurement",
             node_id_);
    break;
  case 0xFF02:
    log_.ERR("MOTOR", "Controller %d error: Phase B current measurement",
             node_id_);
    break;
  case 0xFF03:
    log_.ERR("MOTOR", "Controller %d error: High side FET short circuit",
             node_id_);
    break;
  case 0xFF04:
    log_.ERR("MOTOR", "Controller %d error: Low side FET short circuit",
             node_id_);
    break;
  case 0xFF05:
    log_.ERR("MOTOR", "Controller %d error: Low side phase 1 short circuit",
             node_id_);
    break;
  case 0xFF06:
    log_.ERR("MOTOR", "Controller %d error: Low side phase 2 short circuit",
             node_id_);
    break;
  case 0xFF07:
    log_.ERR("MOTOR", "Controller %d error: Low side phase 3 short circuit",
             node_id_);
    break;
  case 0xFF08:
    log_.ERR("MOTOR", "Controller %d error: High side phase 1 short circuit",
             node_id_);
    break;
  case 0xFF09:
    log_.ERR("MOTOR", "Controller %d error: High side phase 2 short circuit",
             node_id_);
    break;
  case 0xFF0A:
    log_.ERR("MOTOR", "Controller %d error: High side phase 3 short circuit",
             node_id_);
    break;
  case 0xFF0B:
  ...
  case 0xFF0F:
    ...
    break;
  case 0xFF10:
    log_.ERR("MOTOR", "Controller %d error: Controller temperature exceeded",
             node_id_);
    break;
  }
}
...
void Controller::processSdoMessage(utils::io::can::Frame &message) {
  ...
  if (index_1 == 0x27 && index_2 == 0x20 && sub_index == 0x00) {
    if (message.data[4] != 0 && message.data[5] != 0) {
      ...
      case 0x01:
        log_.ERR("MOTOR",
                 "Controller %d warning: Controller temperature exceeded");
        break;
      case 0x02:
      ...
      case 0x40:
        ...
        break;
      default:
        log_.ERR("MOTOR", "Controller %d warning: Warning code %d", node_id_,
                 warning_message);
      }
    }
    return;
  ...
  if (index_1 == 0x41 && index_2 == 0x60 && sub_index == 0x00) {
    ...
    case 0x00:
      ...
      log_.DBG1("MOTOR", "Controller %d state: Not ready to switch on",
                node_id_);
      break;
    case 0x40:
    ...
    case 0x0F:
      ...
      log_.DBG1("MOTOR", "Controller %d state: Fault reaction active",
                node_id_);
      break;
    case 0x08:
      state_ = kFault;
  ...
  if (index_1 == 0x40 && index_2 == 0x20 && sub_index == 0x02) {
    log_.DBG1("MOTOR", "Controller %d: Motor phase offset configured",
              node_id_);
    return;
  }
  if (index_1 == 0x40 && index_2 == 0x20 && sub_index == 0x08) {
    log_.DBG1("MOTOR",
              "Controller %d: Motor phase offset compensation configured",
              node_id_);
    return;
  }
  if (index_1 == 0x54 && index_2 == 0x20 && sub_index == 0x00) {
    log_.DBG1("MOTOR", "Controller %d: Over voltage limit configured",
              node_id_);
    return;
  }
  if (index_1 == 0x55 && index_2 == 0x20 && sub_index == 0x03) {
    log_.DBG1("MOTOR", "Controller %d: Under voltage minimum configured",
              node_id_);
    return;
  }
  if (index_1 == 0x55 && index_2 == 0x20 && sub_index == 0x01) {
    log_.DBG1("MOTOR", "Controller %d: Under voltage limit configured",
              node_id_);
    return;
  }
  if (index_1 == 0x57 && index_2 == 0x20 && sub_index == 0x01) {
    log_.DBG1("MOTOR", "Controller %d: Temperature sensor configured",
              node_id_);
    return;
  }
  if (index_1 == 0x75 && index_2 == 0x60 && sub_index == 0x00) {
    log_.DBG1("MOTOR", "Controller %d: Motor rated current configured",
              node_id_);
    return;
  }
  if (index_1 == 0x76 && index_2 == 0x60 && sub_index == 0x00) {
    log_.DBG1("MOTOR", "Controller %d: Motor rated torque configured",
              node_id_);
    return;
  }
  if (index_1 == 0xF6 && index_2 == 0x60 && sub_index == 0x01) {
    log_.DBG1("MOTOR",
              "Controller %d: Current control torque P gain configured",
              node_id_);
    return;
  }
  if (index_1 == 0xF6 && index_2 == 0x60 && sub_index == 0x02) {
    log_.DBG1("MOTOR",
              "Controller %d: Current control torque I gain configured",
              node_id_);
    return;
  }
  if (index_1 == 0xF6 && index_2 == 0x60 && sub_index == 0x03) {
    log_.DBG1("MOTOR", "Controller %d: Current control flux P gain configured",
              node_id_);
    return;
  }
  if (index_1 == 0xF6 && index_2 == 0x60 && sub_index == 0x04) {
    log_.DBG1("MOTOR", "Controller %d: Current control flux I gain configured",
              node_id_);
    return;
  }
  if (index_1 == 0xF6 && index_2 == 0x60 && sub_index == 0x05) {
    log_.DBG1("MOTOR", "Controller %d: Current control ramp configured",
              node_id_);
    return;
  }
  if (index_1 == 0x50 && index_2 == 0x20 && sub_index == 0x00) {
    log_.DBG1("MOTOR", "Controller %d: Maximum current limit configured",
              node_id_);
    return;
  }
  if (index_1 == 0x51 && index_2 == 0x20 && sub_index == 0x00) {
    log_.DBG1("MOTOR", "Controller %d: Secondary current protection configured",
              node_id_);
    return;
  }
