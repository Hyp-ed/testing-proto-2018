...
FakeController::FakeController(Logger &log, uint8_t id, bool faulty)
    : log_(log), data_(data::Data::getInstance()),
      motor_data_(data_.getMotorData()), node_id_(id), critical_failure_(false),
      actual_velocity_(0), faulty_(faulty), start_time_(0),
      timer_started_(false) {}

void FakeController::registerController() { /*EMPTY*/
}
...
void FakeController::sendTargetVelocity(int32_t target_velocity) {
  ...
  if (counter == 1000) {
    if (node_id_ == 1) {
      RPMvTime << ((timer.getTimeMicros() - start_time_) / 1000) << "\t"
               << target_velocity << "\n";
    }
    counter = 0;
    data::State sm_state_ = data_.getStateMachineData().current_state;
    if (sm_state_ == data::State::kRunComplete ||
        sm_state_ == data::State::kFailureStopped) {
      RPMvTime.close();
    }
  }
  counter++;
...
void FakeController::updateActualVelocity() { /*EMPTY*/
}

void updateActualTorque() { /*EMPTY*/
}

int32_t FakeController::getVelocity() { return actual_velocity_; }
...
void FakeController::healthCheck() {
  // If it is faulty this will choose a random time bewteen the 3 seconds and 23
  // seconds of the run to set critical_failure_ to true
  if (faulty_) {
    data::State state = data_.getStateMachineData().current_state;
    if (state == data::State::kAccelerating ||
        state == data::State::kDecelerating) {
...
bool FakeController::getFailure() { return critical_failure_; }

ControllerState FakeController::getControllerState() { return state_; }
