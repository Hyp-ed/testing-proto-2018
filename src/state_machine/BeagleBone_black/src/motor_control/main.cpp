...
Main::Main(uint8_t id, Logger &log)
    : Thread(id, log), data_(data::Data::getInstance()),
      ...
      prev_velocity_(0), time_of_update_(0), target_velocity_(0),
      prev_index_(0), dec_index_(0), run_(true), nav_calib_(false),
      motors_init_(false), motors_ready_(false), slip_calculated_(false),
      motors_preoperational_(false), motor_failure_(false),
...
Main::transpose(std::vector<std::vector<double>> data) {
...
int32_t Main::accelerationVelocity(NavigationType velocity) {
  ...
  if (upper_bound == acceleration_slip_[0].end())
    upper_bound--;
