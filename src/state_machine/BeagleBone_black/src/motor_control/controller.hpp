...
public:
  ...
  int16_t getTorque();
  /**
   *  @brief { Sets controller into quickStop mode. Use in case of critical
   * failure }
   */
  ...
  /*
   *  @brief { To be called by CAN receive side. Controller processes received
   * CAN message and updates its local data }
   *
...
private:
  ...
  /*
   * @brief { Set critical failure flag to true and write failure to data
   * structure }
   */
  void throwCriticalFailure();
  /*
   * @brief { Sends state transition message to controller, leaving sufficient
   * time for controller to change state. If state does not change, throw
   * critical failure }
   *
