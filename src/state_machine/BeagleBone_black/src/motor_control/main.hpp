...
private:
  /**
   *  @brief  { Establish CAN connections with motor controllers and configure
   * them }
   */
  void initMotors();
  /**
   *   @brief  { Reads slip and translational velocity data from acceleration
   * and deceleration text files, calculates RPM's for appropriate slip at each
   *             translational velocity and stores the values in a 2D array
   * containing translational velocity and RPM }
   */
  void calculateSlip(std::string filepath);
  /**
   *  @brief   { Returns the transposed 2D vector containing acceleration slip
   * and deceleration slip values. This makes it easier to pass the
   * translational velocity vector into binary search }
   */
  std::vector<std::vector<double>>
  transpose(std::vector<std::vector<double>> data);
  /**
   *  @brief  { Set motors into operational state }
   */
  void prepareMotors();
  /**
   *  @brief  { Enter controllers into pre operational state if config error
   * occurs }
   */
   ...
   *
   *  @param[in]  translational_velocity  { Value read from shared data
   * structure }
   *
  ...
  /**
   *  @brief  { Updates the data structure with the velocity and torque of the
   * motors }
   */
