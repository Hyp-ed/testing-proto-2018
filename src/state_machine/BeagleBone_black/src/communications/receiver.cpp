...
ReceiverThread::ReceiverThread(Logger &log, Communications *base_communicator)
    : Thread(), sys_(System::getSystem()), log_(log),
      data_(data::Data::getInstance()),
      base_communicator_(base_communicator) { /* Empty */
}
...
void ReceiverThread::run() {
  ...
  while (sys_.running_) {
    ...
    log_.DBG1(
        "COMN",
        "Before receiving message (launch=%d, reset=%d, length=%.3fm, spg=%d)",
        cmn_.launch_command, cmn_.reset_command, cmn_.run_length,
        cmn_.service_propulsion_go);
    ...
    case 4:
      cmn_.run_length =
          static_cast<float>(base_communicator_->receiveRunLength());
      log_.INFO("COMN", "Received 4 (TRACK LENGTH = %.3fm)",
                cmn_.run_length); // 4: TRACK LENGTH
      break;
    case 5:
      cmn_.service_propulsion_go = true;
      log_.INFO(
          "COMN",
          "Received 5 (SERVICE PROPULSION GO)"); // 5: SERVICE PROPULSION GO
      break;
    case 6:
      cmn_.service_propulsion_go = false;
      log_.INFO(
          "COMN",
          "Received 6 (SERVICE PROPULSION STOP)"); // 6: SERVICE PROPULSION STOP
      break;
    default:
      cmn_.module_status =
          data::ModuleStatus::kCriticalFailure; // DEFAULT: Critical Failure
    ...
    log_.DBG1(
        "COMN",
        "After receiving message (launch=%d, reset=%d, length=%.3fm, spg=%d)",
        cmn_.launch_command, cmn_.reset_command, cmn_.run_length,
        cmn_.service_propulsion_go);
  }
}
