...
Main::Main(uint8_t id, Logger &log)
    : Thread(id, log), sys_(System::getSystem()), log_(log),
...
int Main::getModuleStatusCode(ModuleStatus mod_status) {
  int code = 0;
  switch (mod_status) {
  case ModuleStatus::kStart:
    code = 0;
    break;
  case ModuleStatus::kInit:
    code = 1;
    break;
  case ModuleStatus::kReady:
    code = 1;
    break;
  case ModuleStatus::kCriticalFailure:
    code = 2;
    break;
  default:
    break;
...
int Main::sendState(State state) {
  ...
  case data::kIdle:
    state_code_ = 0;
    break;
  case data::kCalibrating:
    state_code_ = 1;
    break;
  case data::kReady:
    state_code_ = 2;
    break;
  case data::kAccelerating:
    state_code_ = 3;
    break;
  case data::kDecelerating:
    state_code_ = 4;
    break;
  case data::kEmergencyBraking:
    state_code_ = 5;
    break;
  case data::kRunComplete:
    state_code_ = 6;
    break;
  case data::kFailureStopped:
    state_code_ = 7;
    break;
  case data::kExiting:
    state_code_ = 8;
...
int Main::sendHpBattery(Battery hpb) {
  ...
  int i = base_communicator_->sendData(
      "CMD09" + std::to_string(hpb.voltage / 10.0) + "\n");
  int j = base_communicator_->sendData(
      "CMD10" + std::to_string(hpb.current / 10.0) + "\n");
  int k =
      base_communicator_->sendData("CMD11" + std::to_string(hpb.charge) + "\n");
  int l = base_communicator_->sendData("CMD12" +
                                       std::to_string(hpb.temperature) + "\n");
  int m = base_communicator_->sendData(
      "CMD13" + std::to_string(hpb.low_voltage_cell) + "\n");
  int n = base_communicator_->sendData(
      "CMD14" + std::to_string(hpb.high_voltage_cell) + "\n");
...
int Main::sendHpBattery_1(Battery hpb_1) {
  int i = base_communicator_->sendData(
      "CMD15" + std::to_string(hpb_1.voltage / 10.0) + "\n");
  int j = base_communicator_->sendData(
      "CMD16" + std::to_string(hpb_1.current / 10.0) + "\n");
  int k = base_communicator_->sendData("CMD17" + std::to_string(hpb_1.charge) +
                                       "\n");
  int l = base_communicator_->sendData(
      "CMD18" + std::to_string(hpb_1.temperature) + "\n");
  int m = base_communicator_->sendData(
      "CMD19" + std::to_string(hpb_1.low_voltage_cell) + "\n");
  int n = base_communicator_->sendData(
      "CMD20" + std::to_string(hpb_1.high_voltage_cell) + "\n");
...
int Main::sendLpBattery(Battery lpb) {
  int i = base_communicator_->sendData(
      "CMD21" + std::to_string(lpb.voltage / 10.0) + "\n");
  int j = base_communicator_->sendData("CMD22" + std::to_string(lpb.current) +
                                       "\n");
  int k =
      base_communicator_->sendData("CMD23" + std::to_string(lpb.charge) + "\n");
...
int Main::sendLpBattery_1(Battery lpb_1) {
  int i = base_communicator_->sendData(
      "CMD24" + std::to_string(lpb_1.voltage / 10.0) + "\n");
