...
/*
 * @brief    This class is to imitate an IMU. This works by calling the
 * constructor once and calling getData function multiple times at different
 * time periods to produce reading that will be used by other classes.
 */
class FakeImu : public ImuInterface {
 public:
   ...
   *
   *               Sample of the format is located at
   * 'src/fake_imu_input_xxx.txt'. Note that the timestamp for accelerometer has
   * to start with 0 and must be multiples of 250 and 125 for accelerometer and
   * gyroscope respectively. You must include every timestamp from 0 to the last
   * timestamp which will be a multiple of 250 or 125 depending on if it is an
   * accelerometer or gyroscope.
   *
   * @param[in] acc_file_path    A string to the file location of the
   * accelerometer data points
   * @param[in] gyr_file_path    A string to the file location of the gyroscope
   * data points
   */
  FakeImu(utils::Logger& log_, std::string acc_file_path,
          std::string dec_file_path, std::string em_file_path,
          std::string gyr_file_path);

  bool isOnline() override { return true; }
  /*
   * @brief     A function that gets the imu data at the time of call. The
   * function will return the same data point if the time period since the last
   * update isn't long enough. It will also skip a couple of data points if the
   * time since the last call has been sufficiently long.
   */
 ...
 private:
  ...
  /*
   * @brief     A function that reads data from file directory. This function
   * also validates them by checking if 1) The timestamp values are valid.
   * Multiples of 250 or 150 depending on the file. 2) The file follows the
   * format given in the comments of the constructor above. 3) The file exists.
   *
  ...
  void readDataFromFile(std::string acc_file_path, std::string dec_file_path,
                        std::string em_file_path, std::string gyr_file_path);

  /*
   * @brief     A function that adds noise to the imu data using normal
   * distribution
   *
   * @param[in] value    This is the mean of the normal distribution
   * @param[in] noise    This is the standard deviation of the normal
   * distribution
   *
   * @return    Returns random data point value
   */
 public:
  static NavigationVector addNoiseToData(NavigationVector value,
                                         NavigationVector noise);

 private:
  /*
   * @brief     Checks to see if sufficient time has pass for the sensor to be
   * updated and checks if some data points need to be skipped
   */
  bool accCheckTime();
  bool gyrCheckTime();
