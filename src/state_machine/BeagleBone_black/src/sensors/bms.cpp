...
BMS::BMS(uint8_t id, Logger &log)
    : Thread(log), data_({}), id_(id),
      id_base_(bms::kIdBase + (bms::kIdIncrement * id_)), last_update_time_(0),
      can_(Can::getInstance()), running_(false) {
...
bool BMS::hasId(uint32_t id, bool extended) {
  if (!extended)
    return false; // this BMS only understands extended IDs

  // LP BMS CAN messages
  if (id_base_ <= id && id < id_base_ + bms::kIdSize)
    return true;
...
void BMS::processNewData(utils::io::can::Frame &message) {
  ...
  case 0x4: // temperature
    ...
    break;
  default:
    log_.ERR("BMS", "received invalid message, id %d, CANID %d, offset %d", id_,
             message.id, offset);
  }
...
BMSHP::BMSHP(uint16_t id, Logger &log)
    : log_(log), can_id_(id * 2 + bms::kHPBase), local_data_{},
      last_update_time_(0) {
