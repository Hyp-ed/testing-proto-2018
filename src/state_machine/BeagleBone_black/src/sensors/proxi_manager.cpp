...
ProxiManager::ProxiManager(Logger &log, bool is_front,
    ...
    : ProxiManagerInterface(log), sensors_proxi_(proxi),
      i2c_(I2C::getInstance()), is_front_(is_front), is_calibrated_(false) {
  System &sys = System::getSystem();
  old_timestamp_ = utils::Timer::getTimeMicros();
  if (sys.fake_proxi || sys.fake_sensors)
    is_fake_ = true;
