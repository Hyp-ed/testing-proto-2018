...
void printUsage() {
  printf("./hyped [args]\n");
  printf("All arguments are optional. To provide an argument with a value, use "
         "<argument>=<value>.\n"
         "Default value of verbose flags is 1\n"
         "Default value of debug   flags is 0\n"
         "\n  -v, --verbose[=<bool>]\n"
         "    Set system-wide setting of verbosity. If enabled, prints all "
         "INFO messages\n"
         "\n  --verbose_motor, --verbose_nav, --verbose_sensor, "
         "--verbose_state, --verbose_cmn\n"
         "    Set module-specific setting of verbosity. If enabled, prints all "
         "INFO messages\n"
         "\n  -d, --debug[=<level>]\n"
         "    Set system-wide debug level. All DBG[n] where n <= level "
         "messages are printed.\n"
         "\n  --debug_motor, --debug_nav, --debug_sensor, --debug_state, "
         "--debug_cmn\n"
         "    Set module-specific debug level. All DBG[n] where n <= level "
         "messages are printed.\n"
         "\n  --fake_imu --fake_proxi --fake_motors --fake_sensors "
         "--fake_keyence --fake_batteries\n"
         "    Make the system use the fake data drivers.\n"
         "\n  --fail_dec_imu --fail_acc_imu --fail_motors --miss_keyence "
         "--double_keyence\n"
         "    Make the system use the fake data drivers and fail them for "
         "testing.\n"
         "\n  --accurate\n"
...
System::System(int argc, char *argv[])
    : verbose(false), verbose_motor(DEFAULT_VERBOSE),
      verbose_nav(DEFAULT_VERBOSE), verbose_sensor(DEFAULT_VERBOSE),
      verbose_state(DEFAULT_VERBOSE), verbose_cmn(DEFAULT_VERBOSE),
      debug(DEFAULT_DEBUG), debug_motor(DEFAULT_DEBUG),
      debug_nav(DEFAULT_DEBUG), debug_sensor(DEFAULT_DEBUG),
      debug_state(DEFAULT_DEBUG), debug_cmn(DEFAULT_DEBUG), fake_imu(false),
      fake_proxi(false), fake_sensors(false), fake_motors(false),
      fake_embrakes(false), fail_dec_imu(false), fail_acc_imu(false),
      fail_motors(false), miss_keyence(false), double_keyence(false),
      accurate(false), running_(true) {
  int c;
  int option_index = 0;
  while (1) {
    ...
    case 'I':
      if (optarg)
        fail_dec_imu = atoi(optarg);
      else
        fail_dec_imu = 1;
      break;
    case 'j':
      if (optarg)
        fake_proxi = atoi(optarg);
      else
        fake_proxi = 1;
      break;
    case 'J':
      if (optarg)
        fail_acc_imu = atoi(optarg);
      else
        fail_acc_imu = 1;
      break;
    case 'k':
      if (optarg)
        fake_sensors = atoi(optarg);
      else
        fake_sensors = 1;
      break;
    case 'K':
      if (optarg)
        fail_motors = atoi(optarg);
      else
        fail_motors = 1;
      break;
    case 'l':
  ...
  if (verbose_motor == DEFAULT_VERBOSE)
    verbose_motor = verbose;
  if (verbose_nav == DEFAULT_VERBOSE)
    verbose_nav = verbose;
  if (verbose_sensor == DEFAULT_VERBOSE)
    verbose_sensor = verbose;
  if (verbose_state == DEFAULT_VERBOSE)
    verbose_state = verbose;
  if (verbose_cmn == DEFAULT_VERBOSE)
    verbose_cmn = verbose;

  if (debug_motor == DEFAULT_DEBUG)
    debug_motor = debug;
  if (debug_nav == DEFAULT_DEBUG)
    debug_nav = debug;
  if (debug_sensor == DEFAULT_DEBUG)
    debug_sensor = debug;
  if (debug_state == DEFAULT_DEBUG)
    debug_state = debug;
  if (debug_cmn == DEFAULT_DEBUG)
    debug_cmn = debug;
