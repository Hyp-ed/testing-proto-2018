...
/**
 * @brief Calculates an integral. Supports shared output variables that are
 * modifiable by other code as well.
 *
...
class Integrator {
 public:
   ...
   *
   * @param output Where the result is stored. Caller is responsible for freeing
   * the memory
   */
...
DataPoint<T> Integrator<T>::update(const DataPoint<T>& point) {
  ...
  // TODO(Brano,anyone): Change timestamp in DataPoint to use
  // std::chrono::duration Assume timestamp in microseconds and convert to
  // seconds
