...
public:
   ...
   */
  template <typename U> Vector(const Vector<U, dimension> &rhs);
...
template <typename T, int dimension> Vector<T, dimension>::Vector() {
...
template <typename T, int dimension>
Vector<T, dimension> &Vector<T, dimension>::
operator+=(const Vector<T, dimension> &rhs) {
  for (int i = 0; i < dimension; i++)
    elements_[i] += rhs[i];
  return *this;
}
...
template <typename T, int dimension> double Vector<T, dimension>::norm() {
  double ans = 0;
  for (int i = 0; i < dimension; i++)
