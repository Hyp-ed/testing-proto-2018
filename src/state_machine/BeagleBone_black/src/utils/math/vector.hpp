 ...
 * Description: K-dimensional vector class that supports addition, substraction,
...
Vector<T, dimension>::Vector() {
  for (int i = 0; i < dimension; i++) elements_[i] = 0;
}

template <typename T, int dimension>
Vector<T, dimension>::Vector(const T element) {
  for (int i = 0; i < dimension; i++) elements_[i] = element;
}

template <typename T, int dimension>
Vector<T, dimension>::Vector(const std::array<T, dimension>& vector) {
  for (int i = 0; i < dimension; i++) elements_[i] = vector[i];
}

template <typename T, int dimension>
Vector<T, dimension>::Vector(const std::initializer_list<T> elements) {
  for (int i = 0; i < dimension; i++) elements_[i] = *(elements.begin() + i);
}

template <typename T, int dimension>
template <typename U>
Vector<T, dimension>::Vector(const Vector<U, dimension>& rhs) {
  for (int i = 0; i < dimension; i++) elements_[i] = T(rhs[i]);
}
...
Vector<T, dimension>& Vector<T, dimension>::operator+=(
  ...
  for (int i = 0; i < dimension; i++) elements_[i] += rhs[i];
  return *this;
}
...
Vector<T, dimension>& Vector<T, dimension>::operator+=(const T rhs) {
  for (int i = 0; i < dimension; i++) elements_[i] += rhs;
  return *this;
}
...
double Vector<T, dimension>::norm() {
  double ans = 0;
  for (int i = 0; i < dimension; i++) ans += elements_[i] * elements_[i];
  return std::sqrt(ans);
}
...
Vector<T, dimension> Vector<T, dimension>::sqrt() {
  auto ans = *this;
  for (int i = 0; i < dimension; i++) ans[i] = std::sqrt(ans[i]);
  return ans;
}
...
auto operator+(const Vector<T1, dimension>& lhs,
               const Vector<T2, dimension>& rhs)
...
template <typename T1, typename T2, int dimension>
bool operator==(const Vector<T1, dimension>& lhs,
                const Vector<T2, dimension>& rhs) {
