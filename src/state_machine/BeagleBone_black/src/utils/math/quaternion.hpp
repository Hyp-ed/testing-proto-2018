 ...
 * represented as q = a + bi + cj + dk. Internally, the Quaternion is stored as
 * a 4D vector.
 *
...
public:
   ...
   */
  template <typename U> explicit Quaternion(const Vector<U, 4> &rhs);

  template <typename U> Quaternion(const Vector<U, 3> &rhs);
   ...
   */
  template <typename U> Quaternion(const Quaternion<U> &rhs);
  ...
  template <typename U> Quaternion<T> &operator+=(const Quaternion<U> &rhs);
...
template <typename T> Quaternion<T>::Quaternion(const T element) {
  elements_ *= 0;
...
template <typename T> Quaternion<T>::Quaternion(T a, T b, T c, T d) {
  elements_ *= 0;
...
template <typename T> T &Quaternion<T>::operator[](int index) {
  return elements_[index];
}

template <typename T> T Quaternion<T>::operator[](int index) const {
  return elements_[index];
}
...
template <typename T> Quaternion<T> &Quaternion<T>::operator+=(const T rhs) {
  elements_ += rhs;
  return *this;
}
...
Quaternion<T> &Quaternion<T>::operator*=(const Quaternion<T> &rhs) {
  T a = rhs[0] * elements_[0] - rhs[1] * elements_[1] - rhs[2] * elements_[2] -
        rhs[3] * elements_[3];
  T b = rhs[0] * elements_[1] + rhs[1] * elements_[0] - rhs[2] * elements_[3] +
        rhs[3] * elements_[2];
  T c = rhs[0] * elements_[2] + rhs[1] * elements_[3] + rhs[2] * elements_[0] -
        rhs[3] * elements_[1];
  T d = rhs[0] * elements_[3] - rhs[1] * elements_[2] + rhs[2] * elements_[1] +
        rhs[3] * elements_[0];
...
template <typename T> double Quaternion<T>::norm() { return elements_.norm(); }
...
template <typename T> Vector<T, 4> Quaternion<T>::get_elements() const {
  return elements_;
}
