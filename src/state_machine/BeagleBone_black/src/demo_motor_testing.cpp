 ...
 * Organisation: HYPED
 * Date: 28/03/18
 * Description: This demo runs a single motor. It logs RPM, motor temperature
 * and controller temperature data. Both target velocity commands and data
 * updates are executed every 50 milliseconds. The arguments are passed in as
 * follows (Time units are in seconds): Max RPM, Time to accelerate max RPM,
...
void updateVelocityData() {
  ...
  RPMvTime << chrono::duration_cast<chrono::milliseconds>(now - start).count()
           << "\t" << actual_v << "\n";
}
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < acc_iterations; i++) {
    ...
    printf(
        "Motor RPM: %d,    Motor Temperature: %d,    Controller Temperature: "
        "%d\n",
